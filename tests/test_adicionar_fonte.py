"""Testa adicionar_fonte() de mcp_tools.config.
...
config_mod = pytest.importorskip(
    'mcp_tools.config',
    reason='mcp_tools.config não importável (dependências MCP ausentes)',
)
adicionar_fonte = config_mod.adicionar_fonte
...
class TestAdicionarFonteErros:
    def test_id_duplicado(self, monkeypatch):
        monkeypatch.setattr(config_mod, '_load_config', lambda: _config_com_fonte('youtube'))
        monkeypatch.setattr(config_mod, '_save_config', lambda c: None)
    ...
    def test_tipo_invalido(self, monkeypatch):
        monkeypatch.setattr(config_mod, '_load_config', lambda: _config_vazio())
        monkeypatch.setattr(config_mod, '_save_config', lambda c: None)
...
class TestAdicionarFonteSucesso:
    def test_combined_criado_com_sucesso(self, monkeypatch):
        salvo = {}
        monkeypatch.setattr(config_mod, '_load_config', lambda: _config_vazio())
        monkeypatch.setattr(config_mod, '_save_config', lambda c: salvo.update(c))
