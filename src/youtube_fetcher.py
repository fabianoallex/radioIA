...
def _fetch_from_channels(youtube, channels: list[dict], max_per_channel: int,
                          days_lookback: int, max_total: int, lang_pref: list[str]) -> list[dict]:
    videos = []
...
def fetch_content(api_key: str, channels: list[dict], settings: dict, credentials=None) -> list[dict]:
    ...
    target_from_config = max_total - target_from_subs
    ...
    if credentials and target_from_subs > 0:
        ...
        sub_videos = _fetch_from_channels(youtube, sample, 1, days_lookback, target_from_subs, lang_pref)
        for v in sub_videos:
