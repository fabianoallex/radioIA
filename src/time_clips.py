...
_ONES_M = ['zero','um','dois','três','quatro','cinco','seis','sete','oito','nove',
            'dez','onze','doze','treze','quatorze','quinze','dezesseis','dezessete',
            'dezoito','dezenove']
_ONES_F = ['zero','uma','duas','três','quatro','cinco','seis','sete','oito','nove',
            'dez','onze','doze','treze','quatorze','quinze','dezesseis','dezessete',
            'dezoito','dezenove']
...
def generate_atomic_clips(voice: str = 'pt-BR-FranciscaNeural',
                           rate: str  = '+15%',
...
def _trim(audio: 'AudioSegment', trim_start: bool = True, trim_end: bool = True,
          thresh_db: int = -45) -> 'AudioSegment':
    """Remove silêncio inicial e/ou final de um clip."""
    from pydub.silence import detect_leading_silence
    start = detect_leading_silence(audio, silence_threshold=thresh_db, chunk_size=5) if trim_start else 0
    end   = detect_leading_silence(audio.reverse(), silence_threshold=thresh_db, chunk_size=5) if trim_end else 0
