...
TRANSITION_PHRASES = (
    'proximo', 'mudando', 'tem mais', 'olha so', 'agora vamos',
    'continuando', 'falando em', 'e por falar', 'e por ultimo'
)
...
def _overlay_background_music(speech: AudioSegment, music_path: str, volume_db: float) -> AudioSegment:
...
def mix_episode(audio_files: list[str], lines: list[dict], output_path: str,
    ...
    for i, (file_path, line) in enumerate(zip(audio_files, lines)):
        ...
        if i < len(lines) - 1:
            ...
            pause_ms = PAUSE_DIFF_SPEAKER_MS if next_speaker != line['locutor'] else PAUSE_SAME_SPEAKER_MS
