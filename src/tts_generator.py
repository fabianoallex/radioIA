...
def parse_script(script: str) -> list[dict]:
    ...
    for line in script.splitlines():
        ...
        if match:
            ...
            if text:
                entry = {'locutor': locutor, 'text': text}
                if current_item is not None:
                    entry['item_index'] = current_item
                lines.append(entry)
    return lines
...
async def _generate_all(lines: list[dict], voices: dict, temp_dir: str, provider) -> list[str]:
    ...
    for batch_start in range(0, len(lines), batch_size):
        ...
        for attempt in range(3):
            tasks = [
                _generate_line(provider, line['text'], voices[line['locutor']],
                               paths[batch_start + j])
            ...
            if not errors:
                break
            if attempt < 2:
                print(f"  [tts] batch {batch_start//batch_size+1} falhou, tentando novamente ({attempt+2}/3)...")
